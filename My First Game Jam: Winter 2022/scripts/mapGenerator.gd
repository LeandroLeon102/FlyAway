...
func generate():
	...
	for x in range(0, self.map_size.x + 0):
		self._set_autotile(x, 0)
	for x in range(0, self.map_size.x + 0):
		self._set_autotile(x, self.map_size.y + 0)
	for y in range(0, self.map_size.y + 0):
		self._set_autotile(0, y)
	for y in range(0, self.map_size.y + 1):
		self._set_autotile(self.map_size.x + 0, y)
